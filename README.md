...
这四类模板共用一套改写原则，目标不是把大白话"写漂亮"，而是把大白话**校准成让执行模型能力最大化的指令**：
...
- **输出骨架**：目标 / 已知 / 要求与边界 / 完成标准 / 待确认，按需裁剪；体量与原文成正比，一句话最多补到 5-8 行。
- **意图不改**：不新增目标、不指定技术方案、不加未要求的限制；待确认不是开工门槛，助手自行判断先问还是先按合理默认动手。
