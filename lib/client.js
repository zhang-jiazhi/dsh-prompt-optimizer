	...
	factory: (require) => {
		...
		function PromptOptimizerBar(props) {
			...
			function onButtonClick() {
				...
				var request = { controller: controller, original: draft, sessionId: props.sessionId, canceled: false };
				...
				fetch(OPTIMIZE_URL, {
					...
					.then(function (body) {
						...
						if (body && body.ok && typeof body.text === 'string' && body.text.length > 0) {
							...
							setState({ sessionId: request.sessionId, original: request.original, optimized: body.text });
							setAnnounce('提示词已优化，再次点击可恢复原文');
		...
		var SETTINGS_FIELDS = [
			...
			{ key: 'reasoningEffort', label: '推理强度', hint: 'inherit=跟随模型默认；调低可显著加快优化速度', type: 'select',
