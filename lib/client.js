...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * slots 是必需服务；设置表单服务按宿主版本二选一（0.1.7 的 configForms，
		 * 或旧宿主的 settingsScope），两者都是可选依赖 —— 拿不到只降级设置分区，
		 * 输入框工具行必须始终可用，因此都不放进 inject 门控。
		 */
		var inject = ['slots'];
		...
		/** 页面级模板目录缓存：5 分钟 TTL，过期重取（挂载 effect），失败重试绕过缓存。
		 *  永不失效会让一次坏目录/半截响应固化到页面关闭（二轮 P3）。 */
		var catalogCache = null;
		var catalogCacheAt = 0;
		...
		/**
		 * 按草稿特征预选模板（只改默认值，不锁定；用户随时可换）。
		 *
		 * 动机：插件有 15 个模板、默认项对所有输入都套「六要素任务书」
		 * （intent-rules 输出骨架 + task-strength 完整度），而「简单消息保持简单」
		 * 这条逃生条款只写在上下文类模板里。结果是把「帮我写个正则」这类
		 * 一句话需求也结构化成任务书，接收模型会按任务书交报告。
		 *
		 * 判据是纯启发式字符串匹配，零模型调用：
		 *   - 带「那个 / 它 / 刚才」等指代 → 上下文类（消歧是它唯一的价值）
		 *   - 画面词（画 / 图 / 镜头 / 光影…）→ 图像类
		 *   - 代码痕迹（路径 / 命令 / 扩展示例 / 代码块 / 报错）→ 任务指令优化
		 *   - 逆向安全词 → 安全研究语境模板
		 *   - 其余短草稿（≤ 30 字且无约束）→ 仍用默认，但由轻量档控制体量
		 * 只在用户没手动选过该类别时生效（byCategory 里没有记录）。
		 */
		function suggestTemplateId(templates, draft) {
			var text = typeof draft === 'string' ? draft : '';
			if (text.trim() === '') return null;
			var firstOfCategory = function (category) {
				var item = templates.find(function (t) { return t.category === category; });
				return item === undefined ? null : item.id;
			};
			var byId = function (id) {
				return templates.some(function (t) { return t.id === id; }) ? id : null;
			};
			var contextHint = /那个|这个|它|刚才|上次|前面说|之前说|之前提到/;
			var imageHint = /(?:画|绘制|出图|插画|海报|配图|封面|镜头|光影|构图|景深|水墨|油画|水彩|写实|赛博|立绘|三视图|图生图|换背景|文生图|image|photo)/i;
			var secureHint = /(?:逆向|破解|脱壳|抓包|改包|爆破|撞库|提权|免杀|内存马|WebShell|后门|木马|钓鱼|社工|漏洞|注入|Hook|外挂|注册机|keygen|CTF|靶机|靶场|取证|红蓝|Pwn|ROP|UAF)/i;
			var codeHint = /(?:```|\.(?:js|ts|tsx|jsx|py|go|rs|java|c|cpp|h|json|ya?ml|md|sh|zsh|css|html|vue|rb|php)\b|(?:^|[\s(])\/(?:Users|home|var|etc|tmp)\/|\b[a-z]+\.(?:js|py|go|rs|java)\b|npm\b|pnpm\b|yarn\b|pip\b|cargo\b|git\b|报错|异常|堆栈|traceback|bug|修(?:一下|复)|重构|函数|接口|脚本)/i;
			if (secureHint.test(text)) return byId('secure-reverse-optimize');
			if (imageHint.test(text)) return firstOfCategory('image');
			if (contextHint.test(text)) return byId('context-message-optimize') ?? firstOfCategory('context');
			// 代码/技术类：显式指向任务指令优化，不要靠"basic 第一条"这种巧合
			// （目录顺序一改就静默失效）。
			if (codeHint.test(text)) return byId('user-task-optimize') ?? firstOfCategory('basic');
			return null;
		}

		/**
		 * 约束账本摘要：把宿主返回的 ledger 统计渲染成一句人话。
		 * 只在真的发生漂移（删除或新增）时才显示，避免每次优化都多一行噪音。
		 */
		function formatLedgerSummary(ledger) {
			if (ledger === null || typeof ledger !== 'object') return null;
			var dropped = typeof ledger.dropped === 'number' ? ledger.dropped : 0;
			var invented = typeof ledger.invented === 'number' ? ledger.invented : 0;
			if (dropped === 0 && invented === 0) return null;
			var parts = [];
			if (dropped > 0) parts.push('约束丢失 ' + dropped);
			if (invented > 0) parts.push('新增约束 ' + invented);
			return '约束核对：' + parts.join(' · ');
		}
		...
		function PromptOptimizerBar(props) {
			...
			//
			// Rules of Hooks（二轮 P2）：props.useInput 是宿主注入的 hook，条件调用会让
			// hook 链在两次渲染间变长/变短（React 直接报错）。首次渲染用 useRef 锁定
			// 引用——之后每次渲染都调用同一个引用，hook 链稳定；prop 后续出现/消失都
			// 不改变调用与否。首渲染没有就是整个生命周期都不调用（与 props.input 回退
			// 兼容），这与「useRef 只在挂载时取初值」的语义天然一致。
			var useInputRef = React.useRef(props.useInput);
			var inputSnapshot = typeof useInputRef.current === 'function'
				? useInputRef.current(function (state) { return state; })
				: null;
			...
			if (stateForSession && state.busy !== true) {
				...
				// 约束账本摘要排在质量提示之前：它回答"我的约束还在不在"，
				// 是用户最关心的第一件事；没有漂移时 formatLedgerSummary 返回 null。
				var ledgerSummary = formatLedgerSummary(state.ledger);
				if (ledgerSummary !== null) noticeParts.push(ledgerSummary);
				if (Array.isArray(state.warnings)) {
					// 守卫现在有 7 种 alert code，一次优化可能同时命中多条；旧实现
					// 硬切前 2 条，第 3 条起静默丢弃。这里按 code 去重后最多展示 4 条，
					// 余下折叠成一条计数提示。
					var seenWarningCodes = {};
					var warningMessages = [];
					state.warnings.forEach(function (warning) {
						if (!warning || typeof warning.message !== 'string') return;
						var warningCode = typeof warning.code === 'string' ? warning.code : warning.message;
						if (seenWarningCodes[warningCode] === true) return;
						seenWarningCodes[warningCode] = true;
						warningMessages.push(warning.message);
					});
					warningMessages.slice(0, 4).forEach(function (message) {
						noticeParts.push('⚠ ' + message);
					});
					if (warningMessages.length > 4) {
						noticeParts.push('⚠ 另有 ' + (warningMessages.length - 4) + ' 条质量提示');
					}
				}
			...
			// 拉取模板目录（模块级缓存带 TTL：TTL 内的会话切换/组件重挂载不重复
			// 请求；过期重取；失败时给出可见错误并可点击重试）。
			React.useEffect(function () {
				if (catalogCache !== null && Date.now() - catalogCacheAt < CATALOG_CACHE_TTL_MS) {
			...
			function pickCategory(e) {
				...
				setSel(function (prev) {
					...
					var rememberedValid = categoryTemplatesOf(templates, category).some(function (t) { return t.id === remembered; });
					// 用户没手动选过这个类别时，先按当前草稿预选一个更合适的模板；
					// 预选不到就退回该类别默认项。用户选过（remembered 有效）则不覆盖。
					var suggested = rememberedValid ? null : suggestTemplateId(templates, draftRef.current);
					var suggestedFits = suggested !== null && categoryTemplatesOf(templates, category).some(function (t) { return t.id === suggested; });
					var id = rememberedValid
						? remembered
						: (suggestedFits ? suggested : defaultTemplateId(templates, category));
			...
			/** 写回草稿：宿主缺 inputActions.setDraft 时只告警降级、不抛
			 *  （撤销/写回不可用，但工具行其余功能照常——二轮 P2）。 */
			function writeDraft(value) {
				var setDraft = props.inputActions && props.inputActions.setDraft;
				if (typeof setDraft !== 'function') {
					console.warn('dsh-prompt-optimizer: 宿主未提供 inputActions.setDraft，本次结果无法写回输入框');
					return;
				}
				setDraft(value);
			}
				...
				fetch(OPTIMIZE_URL, {
					...
					.then(function (body) {
						...
						if (body !== null && body.ok === true && typeof body.text === 'string' && body.text.length > 0) {
							...
							var ledger = body.ledger !== null && typeof body.ledger === 'object' ? body.ledger : null;
							var stats = formatStats(body.ms, body.usage);
							setState(Object.assign(
								{ sessionId: request.sessionId, original: request.original, optimized: body.text, warnings: warnings, ledger: ledger, stats: stats },
							...
							if (ledger !== null && (ledger.dropped > 0 || ledger.invented > 0)) announcement += '；约束核对有差异';
				...
				return React.createElement(
					...
					React.createElement('button', {
						...
						onClick: function () {
							...
							catalogCache = null; // 失败重试必须绕过缓存，不能拿旧目录冒充成功
							setCatalogError(null);
		...
		// 设置 → 插件 的插件卡片：读写本插件的设置命名空间。
		// 0.1.7 规范：命名空间 = Host profile 里的 entry id（不再是插件自报的名字），
		// 表单由宿主 configForms 按 entry 提供；旧宿主仍用 settingsScope + 自报名字。
		var SETTINGS_ENTRY_ID = 'dsh-prompt-optimizer-host';
		...
		var SETTINGS_FIELDS = [
			...
			{ key: 'timeoutMs', label: '单次优化超时（毫秒）', hint: '前端输入框下限 1000；服务端实际允许 10ms 起（测试用途，亚秒级超时也可生效）', type: 'number', min: 1000, step: 1000 },
				...
				if (def.type === 'select') {
					...
					input = React.createElement('input', {
						...
						onBlur: function (e) {
							...
							commit(def.key, raw, def);
							// 失焦后把显示值同步为规范化结果（二轮 P3）：onChange 先写入了
							// 原始输入，空/非法输入被 commit 忽略时若不同步，输入框会一直
							// 停在坏输入上、与实际生效的设置脱节。仅数字字段需要归一。
							if (def.type !== 'number') return;
							setFormValues(function (prev) {
								var next = Object.assign({}, prev);
								var canonical = typeof raw === 'string' && raw.trim() !== '' && Number.isFinite(Number(raw))
									? Number(raw)
									: resolved[def.key];
								if (canonical === undefined) delete next[def.key];
								else next[def.key] = canonical;
								return next;
							});
						},
			...
			return React.createElement(
				...
				scope === null
					? React.createElement('p', { className: 'dpo-set-warn' },
						'设置表单不可用：当前宿主未提供设置表单服务（configForms），本次使用内置默认值，优化功能不受影响。')
					: null,
				rows,
			);
		}
		 ...
		 * 在宿主客户端上下文里查一个可选服务。
		 *
		 * 真实 cordis 子作用域同时提供 `get(name)` 与属性代理；部分 facade / 测试
		 * 替身只有其中一种。属性访问在「服务未声明」时可能直接抛错（9-19 的教训：
		 * 未声明即抛会让整个客户端包 failed），所以这里两种都试、且吞掉异常。
		 * @param scope - 客户端根上下文或注入子作用域。
		 * @param serviceName - 服务名。
		 * @returns 服务实例，或 undefined。
		 */
		function lookupService(scope, serviceName) {
			if (scope === null || scope === undefined) return undefined;
			if (typeof scope.get === 'function') {
				var viaGet = scope.get(serviceName);
		 ...
		 * 从宿主客户端上下文解析设置表单提供者。
		 *
		 * 0.1.7 规范：`ctx.configForms.get(entryId)` 直接返回表单控制器
		 * （getSnapshot/subscribe/set，命名空间 = Host profile entry id）。
		 * 为了让组件内既有的 `binder.bind({namespace})` 调用点保持不变，这里把
		 * configForms 适配成同形状的 `{ bind }`：bind 忽略传入的旧命名空间名，
		 * 一律按 entry id 取表单。
		 * 旧宿主回落：`ctx.settingsScope.bind({namespace})` 原样返回。
		 * 两者都拿不到时返回 null —— 只降级设置分区，输入框工具行不受影响。
		 * @param scope - 客户端根上下文或其注入子作用域。
		 * @returns 设置表单提供者，或 null。
		 */
		function resolveBinderFrom(scope) {
			var configForms = lookupService(scope, 'configForms');
			if (configForms !== null && configForms !== undefined && typeof configForms.get === 'function') {
				return {
					bind: function () {
						try {
							return configForms.get(SETTINGS_ENTRY_ID);
						} catch (error) {
							console.error('dsh-prompt-optimizer: configForms.get failed', error);
							return null;
						}
					},
				};
			}
			var settingsScope = lookupService(scope, 'settingsScope');
			return settingsScope !== null && settingsScope !== undefined ? settingsScope : null;
		}
		...
		function apply(ctx) {
			...
			// 设置表单服务跨宿主版本二选一（0.1.7 的 configForms / 旧宿主的
			// settingsScope）。两者都是可选依赖：拿不到只降级设置分区，
			// 输入框工具行必须始终可用，因此都不进 inject 门控。
			updateSettingsBinder(resolveBinderFrom(ctx));
			try {
				if (typeof ctx.inject === 'function') {
					ctx.inject(['configForms'], function (scoped) {
						updateSettingsBinder(resolveBinderFrom(scoped || ctx));
					});
