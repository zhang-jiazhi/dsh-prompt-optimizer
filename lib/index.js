 ...
 *
 * 设置（0.1.7 规范，2026-09-22 迁移）：不再调用已删除的 `ctx.settings.register()`，
 * 改为导出 `Config`（schemastery schema），全部参数标 `.volatile()` —— 宿主
 * dsh-settings 的 configEditor 据此投影出可编辑表单（namespace = 本插件的
 * profile entry id `dsh-prompt-optimizer-host`），改动即时生效并持久化到
 * profile patch。本插件自建「设置 → 提示词优化」分区页，故用
 * `settings.configure({ auto: false })` 抑制宿主再自动生成一份重复页面。
 * 旧宿主（无 configForms/volatile 语义）下走 schema 默认值，优化功能不受影响。
 */
...
/**
 * volatile 配置引用的判定：宿主 cosmokit 用全局注册符号
 * `Symbol.for('cosmokit.volatile.write')` 标记引用（createVolatile 只挂
 * `get()` 与该符号方法）。这里用同一个全局符号做等价判定，避免为本插件
 * 新增 @deepseek-ai/cosmokit 依赖 —— 该符号跨 ESM/CJS 副本一致，判定与
 * cosmokit 的 isVolatile 完全等价。
 */
...
/**
 * 插件设置 schema 工厂（设置 → 插件 → 提示词优化器）。
 *
 * schemastery 是本插件唯一的外部依赖，改为在 apply() 内动态加载：
 * 依赖缺失时只降级设置页（走内置默认值），不再让整个插件树加载失败。
 * @param {object} z - schemastery 命名空间（含 object/string/number/natural）。
 */
export function buildConfigSchema(z) {
	return z.object({
		provider: z.string().default('').volatile().description('模型提供方（留空 = 跟随 DSH 默认模型）'),
		model: z.string().default('').volatile().description('模型 ID（留空 = 跟随 DSH 默认模型；与提供方需同时配置）'),
		temperature: z.number().min(0).max(2).step(0.1).default(0.3).volatile().description('采样温度'),
		reasoningEffort: z.string().default('inherit').volatile().description('推理强度：inherit=不指定，由模型/适配器默认决定（推荐；优化是轻量任务，跟随主模型的 max 档会明显变慢）；off/low/medium/high/max 为显式覆盖；模型不支持该档位时本次优化自动回退到模型默认档位重试'),
		maxTokens: z.natural().min(MIN_MAX_TOKENS).max(MAX_MAX_TOKENS).default(8192).volatile().description('单次输出 token 上限（推理型模型会把思考 token 计入同一预算，留足余量；预算被思考吃光时本插件会按更宽预算自动重试一次）'),
		timeoutMs: z.natural().min(MIN_TIMEOUT_MS).default(120000).volatile().description('单次优化超时（毫秒）'),
		maxInputChars: z.natural().min(MIN_MAX_INPUT_CHARS).max(MAX_MAX_INPUT_CHARS).default(8000).volatile().description('待优化文本长度上限（字符，超长拒绝）'),
		contextMaxMessages: z.natural().max(MAX_CONTEXT_MAX_MESSAGES).default(12).volatile().description('上下文类模板携带的最近对话条数'),
		contextMaxChars: z.natural().max(MAX_CONTEXT_MAX_CHARS).default(4000).volatile().description('上下文文本总长上限（字符；0=关闭上下文携带；预算不足时优先保留最新对话）'),
	}).description('提示词优化器（移植自 prompt-optimizer）');
}

/** 模板 id → 定义。 */
/** 模板目录按 id 索引。每次现取（getTemplates 内部按 mtime 缓存），改模板免重载。 */
function templateMap() {
	return new Map(getTemplates().map((template) => [template.id, template]));
}
...
/** 输出 token 地板值：低于此值必然被推理型模型的思考 token 吃光，纯属徒劳重试。 */
const MIN_MAX_TOKENS = 64;
/** 超时地板值：亚秒级超时是受支持特性（测试覆盖 timeoutMs=50），只挡 1ms 级病态值。 */
const MIN_TIMEOUT_MS = 10;
/** 输入长度地板值：与设置页 UI 的 min 一致——floor=1 时任何输入都会被 400（二轮 P3）。 */
const MIN_MAX_INPUT_CHARS = 200;

/**
 * 各项上限（天花板）。
 *
 * 原来这几个设置项**只有下界没有上界**：把「上下文文本总长上限」填成 1e8 后，
 * `perMessageLimit` 被 CONTEXT_MESSAGE_CLIP_CHARS 顶到 2000、条数被
 * DIALOG_COLLECT_CAP 顶到 200 → 单次请求最多携带 200×2000 = 40 万字符
 * （≈10 万+ token）上下文，且 fitContextBudget 因 `full.length <= maxChars`
 * 提前返回而**完全不截断**，每次点击都发。timeoutMs 早已有 MAX_TIMEOUT_MS
 * 钳制，这几项属同类防护缺失。
 */
const MAX_MAX_TOKENS = 65536;
const MAX_MAX_INPUT_CHARS = 200000;
const MAX_CONTEXT_MAX_MESSAGES = 200;
const MAX_CONTEXT_MAX_CHARS = 20000;

/**
 * 0.1.7 规范的插件配置声明：宿主 dsh-settings 的 configEditor 读取本 schema，
 * 把 `.volatile()` 字段投影为设置页表单（namespace = profile entry id）。
 * 字段全部即时生效，因此全部 volatile；无 volatile 字段的插件不会出现在设置页。
 * 位置约束：必须排在 schema 引用的上下界常量之后 —— 顶层 `const` 一旦提前调用
 * `buildConfigSchema` 就会撞上这些常量的 TDZ（Cannot access before initialization）。
 */
export const Config = buildConfigSchema(z);

function positiveIntOr(value, fallback, floor = 1, ceiling = Number.MAX_SAFE_INTEGER) {
	return Math.min(ceiling, Math.max(floor, Math.floor(numOr(value, fallback))));
}

function nonNegativeIntOr(value, fallback, ceiling = Number.MAX_SAFE_INTEGER) {
	return Math.min(ceiling, Math.max(0, Math.floor(numAtLeast0(value, fallback))));
}

/** setTimeout 在 Node 与浏览器都能接受的最大安全延迟。 */
const MAX_TIMEOUT_MS = 2_147_483_647;

function timeoutOr(value, fallback, floor = MIN_TIMEOUT_MS) {
	const number = numOr(value, fallback);
	return Math.min(MAX_TIMEOUT_MS, Math.max(floor, Math.floor(number)));
}
 ...
 *
 * 必须容忍空白变体：`</对话上下文 >`、`</ 对话上下文>` 在模型眼里同样是
 * 闭合标签，只转义标准写法等于留了个口子。
 */
function sanitizeContextText(text) {
	return text.replace(/<\s*\/\s*(对话上下文)\s*>/g, '<\\/$1>');
}

/** 单条上下文消息的字符上限：长消息保留开头和结尾，结论通常在尾部。 */
const CONTEXT_MESSAGE_CLIP_CHARS = 2000;
const CONTEXT_CLIP_MARKER = '…（中间省略）…';

function clipContextMessage(text, limit = CONTEXT_MESSAGE_CLIP_CHARS) {
	if (text.length <= limit) return text;
	const head = Math.floor(limit * 0.6);
	const tail = Math.max(0, limit - head - CONTEXT_CLIP_MARKER.length);
	return `${text.slice(0, head)}${CONTEXT_CLIP_MARKER}${tail > 0 ? text.slice(-tail) : ''}`;
}

/**
 * 按字符预算组装上下文：预算不足时从最新一条往回装。
 *
 * 这里刻意不沿用「join 后 slice(0, N)」的写法——那会保住最旧的消息、丢掉
 * 最新的对话，而用户说「刚才 / 那个 / 上一条」时，最新的才是消歧依据。
 * 最新一条自身超预算时保留它的尾部（最近的措辞与结论），而不是整条丢弃。
 */
function fitContextBudget(texts, maxChars) {
	const marker = '…（已截断）';
	// 截断标记本身也要占预算，否则输出会稳定超出 maxChars。
	...
	for (let index = texts.length - 1; index >= 0; index -= 1) {
		...
		// 「[用户] / [助手]」标签是消歧依据：模板要求区分"用户已确认的事实"与
		// "助手自己过去的做法"，标签被切掉就会把助手的猜测当成用户确认搬进输出。
		// 截断只作用于正文，标签必须留在预算内。
		const label = /^\[(?:用户|助手)\]\s?/.exec(piece)?.[0] ?? '';
		const body = piece.slice(label.length);
		// 连标签都放不下时只能整条放弃：room 必须允许 <= 0 并在此退出。
		// 原实现写 `Math.max(0, ...)`，room=0 时 `body.slice(-0) === body.slice(0)`
		// 会把**整条超长正文**原样塞进上下文（实测 27 字预算 → 输出 527 字，
		// 约 20 倍超限），直接打破"输出不超预算"不变量。
		const room = remaining - label.length;
		if (room <= 0) break;
		parts.unshift(`${label}${body.slice(-room)}`);
		break;
	}
	const joined = parts.join('\n');
	// 预算比截断标记还小：宁可不加标记，也不能让输出超过 maxChars。
	if (marker.length >= maxChars) return joined.slice(0, Math.max(0, maxChars));
	return `${joined}${marker}`;
}

/** 只保留可序列化的 token 统计字段，避免把适配器私有结构透给客户端。 */
...
async function gatherContext(ctx, sessionId, cfg, signal) {
	...
	try {
		...
		// 单条上限跟着预算走：固定 2000 时，12 条 × 2000 = 24000 字潜在输入，
		// 对默认 4000 字预算是 6 倍，几乎每次调用都会走有损截断。让单条最多占
		// 预算的三分之一，条数与预算才在同一个量级。
		const perMessageLimit = Math.max(200, Math.min(CONTEXT_MESSAGE_CLIP_CHARS, Math.floor(cfg.contextMaxChars / 3)));
		const texts = collected
			.slice(-cfg.contextMaxMessages)
			.map(({ role, text }) => {
				const clipped = clipContextMessage(text, perMessageLimit);
				return `[${role === 'user' ? '用户' : '助手'}] ${sanitizeContextText(clipped)}`;
			});
		if (texts.length === 0) return emptyContext(CONTEXT_FIRST_MESSAGE);
		const joined = fitContextBudget(texts, cfg.contextMaxChars);
...
/**
 * 输出清尾：剥掉模型给整条提示词套的代码围栏。
 *
 * 判据只有一条：**没有歧义才动手**。
 *   1. 首尾成对，且**内部围栏自身配对** → 脱壳；
 *   2. 全文只有唯一一处围栏 → 那是残留的半截围栏，按位置剥掉；
 *   3. 其余（内部围栏落单、或围栏 ≥ 2 处且未成对）→ 一个字都不动。
 *
 * 第 1 条的"内部配对"校验是踩过两次坑补上的：只看首尾成对就脱壳，会把
 * 「外层围栏 + 正文里的代码块」中的代码块开围栏当成内容剥掉，制造出未闭合
 * 代码块（`示例：```js foo()`）。宁可留一处残留围栏，也不能破坏正文结构。
 */
function stripCodeFence(text) {
	const paired = /^\s*```[^\n]*\n([\s\S]*?)\n?```\s*$/.exec(text);
	if (paired !== null) {
		const inner = paired[1];
		const innerFences = (inner.match(/^[ \t]*```/gm) ?? []).length;
		return innerFences % 2 === 0 ? inner : text;
	}
	const fenceCount = (text.match(/^[ \t]*```/gm) ?? []).length;
	if (fenceCount !== 1) return text;
...
function isUnsupportedEffortFailure(failure) {
	...
	// 中英双写（二轮 P3）：部分适配层会把这类拒绝包装成中文文案，只认英文
	// 会漏掉回退机会，让一次本可自愈的失败直接变成报错。
	return /reasoning effort/i.test(message) || /推理强度|不支持.{0,12}强度/.test(message);
}

/** 单次流式调用：聚合文本、结束原因与 token 用量。 */
...
async function runOptimize(ctx, cfg, template, text, contextText, signal) {
	...
	const message = {
		...
		source: { kind: 'plugin:dsh-prompt-optimizer' },
	};
	const options = {
	...
	// 推理强度：inherit（默认）= 不传该字段，由适配器/模型默认决定。刻意不跟随
	// agent 默认模型的档位——优化是轻量任务，跟随主模型的 max 档会让每次点击
	// 多等十几秒；只有显式档位才覆盖，且模型不支持时允许回退一次。
	let overrodeEffort = false;
	if (REASONING_EFFORTS.has(cfg.reasoningEffort)) {
		options.reasoningEffort = cfg.reasoningEffort;
		// 只要**显式设置了**档位就允许回退一次。原实现写
		// `overrodeEffort = route.reasoningEffort !== cfg.reasoningEffort`，
		// 于是「设置值恰好等于路由默认档位」（例如两边都是 off / max）时
		// overrodeEffort 恒为 false → retryable 恒 false → 注释里承诺的
		// 「不支持该档位时按模型默认档位重试一次」这条自愈路径完全不生效，
		// 一次本可自愈的失败变成硬报错。
		overrodeEffort = true;
	}
	...
	let usage = null;
	try {
		({ out, finish, usage } = await streamOnce(llm, currentOptions));
	} catch (error) {
		// 适配层可能直接抛（而不是产出 finish 块）。
		if (!retryable(error)) throw error;
		({ out, finish, usage } = await retryWithoutOverride());
		currentOptions = withoutEffortOverride();
	}
	...
	// effectiveMaxTokens 记录实际生效的预算：重试后再失败时，文案要引用真正用过的
	// 值而不是原始 cfg.maxTokens，否则用户会去调一个已经不起作用的参数（二轮 P3）。
	...
	if (finish?.kind === 'max-tokens' && out.trim() === '' && !signal?.aborted && cfg.maxTokens < BUDGET_RETRY_CEILING) {
		...
		effectiveMaxTokens = widened;
		ctx.logger?.warn?.(`dsh-prompt-optimizer: 输出预算 ${cfg.maxTokens} 被思考 token 吃光，按 ${widened} 重试一次`);
		({ out, finish, usage } = await streamOnce(llm, { ...currentOptions, maxTokens: widened }));
	}
	...
	if (kind === 'max-tokens') throw new Error(`输出达到 token 上限（${effectiveMaxTokens}）；推理型模型的思考 token 也算在这个预算里，可在设置页调大「单次输出 token 上限」或调低推理强度`);
	if (out.trim() === '') throw new Error('模型没有返回文本');
	// 模板自检之外的第二道防线：只检测与轻量规范化，不自动重试。
	const guarded = validateOptimizedOutput({ template, draft: text, output: stripCodeFence(out.trim()) });
	return { text: guarded.text, warnings: guarded.warnings, ledger: guarded.ledger, usage: usageSummary(usage) };
}

/**
 * 配置就绪信号（测试钩子）：0.1.7 迁移后配置由宿主在装配时按 `Config` schema
 * 解析并传入 `apply`，不再有异步注册阶段，因此恒为已 resolve。
 */
export let settingsReady = Promise.resolve();

function apply(ctx, pluginConfig) {
	// 0.1.7 规范：不再注册命名空间（`settings.register` 已从宿主删除）。宿主
	// configEditor 从本模块导出的 `Config` 投影设置表单；本插件自带「设置 →
	// 提示词优化」分区页（客户端 half），故显式关闭宿主的自动页面生成，避免
	// 同一份配置出现两个入口。
	...
	/**
	 * 读取活配置：0.1.7 里 `.volatile()` 字段在 config 上是 `{ get(), write() }`
	 * 引用（改动即时生效、不重挂载），读取前必须解包；非 volatile 字段原样返回。
	 */
	const unwrap = (value) => (value !== null && typeof value === 'object' && VOLATILE_WRITE in value ? value.get() : value);
	const config = () => {
		try {
			const source = pluginConfig === undefined || pluginConfig === null || typeof pluginConfig !== 'object' ? {} : pluginConfig;
			const out = {};
	...
	ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			writeJson(res, 200, {
				ok: true,
				templates: getTemplates().map(({ id, name: templateName, desc, category }) => ({ id, name: templateName, desc, category })),
			});
		},
	});
	...
	ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			const cfg = {
				...
				maxTokens: positiveIntOr(raw.maxTokens, DEFAULT_MAX_TOKENS, MIN_MAX_TOKENS, MAX_MAX_TOKENS),
				timeoutMs: timeoutOr(raw.timeoutMs, 120000),
				maxInputChars: positiveIntOr(raw.maxInputChars, MAX_INPUT_CHARS_FALLBACK, MIN_MAX_INPUT_CHARS, MAX_MAX_INPUT_CHARS),
				contextMaxMessages: Math.min(DIALOG_COLLECT_CAP, nonNegativeIntOr(raw.contextMaxMessages, 12, MAX_CONTEXT_MAX_MESSAGES)),
				contextMaxChars: nonNegativeIntOr(raw.contextMaxChars, 4000, MAX_CONTEXT_MAX_CHARS),
			};
			/** 统一的超时响应：body / 上下文 / 模型三个阶段共用同一份可操作文案。 */
			const timeoutResponse = () => {
				// 秒数向上取整并至少显示 1 秒：Math.round 会把 <500ms 的超时说成
				// 「0 秒」，用户看不懂也没法据此调参。
				const seconds = Math.max(1, Math.ceil(cfg.timeoutMs / 1000));
				return { ok: false, error: 'timeout', message: `优化超时（${seconds} 秒），可在「设置 → 提示词优化」调大超时或调低推理强度` };
			};

			// deadline 提前到 body 读取之前（二轮 P3）：慢滴流的 body 同样受 timeoutMs
			// 约束，不再只有模型阶段有超时。deadline.signal 是 controller.signal 的子
			// 信号（二者已合并），客户端断开照常穿透 readBody，无需再传原 signal。
			const startedAt = Date.now();
			const deadline = createDeadline(controller.signal, cfg.timeoutMs);

			let body;
			try {
				body = await readBody(req, 256 * 1024, deadline.signal);
			} catch (error) {
				deadline.dispose();
				if (deadline.signal.reason?.name === 'TimeoutError') {
					detachResponseClose();
					return writeJson(res, 200, timeoutResponse());
				}
			...
			try {
				// 同一个 deadline 覆盖 body 读取、上下文读取和模型调用；否则慢的
				// sessionQuery 会绕过“单次优化超时”，并在用户取消后继续阻塞 handler。
				...
				writeJson(res, 200, {
					ok: true,
					text: optimized.text,
					// 模板自检之外的第二道防线：占位符丢失/角色卡泄漏/注水等只告警，
					// 由客户端提示用户，不自动重试（避免延迟与成本翻倍）。
					warnings: optimized.warnings,
					// 约束账本统计（保留/删除/新增），客户端据此显示变更摘要。
					ledger: optimized.ledger,
