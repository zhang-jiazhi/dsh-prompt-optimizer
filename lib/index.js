...
export function buildConfigSchema(z) {
	return z.object({
		...
		contextMaxChars: z.natural().default(4000).description('上下文文本总长上限（字符；预算不足时优先保留最新对话）'),
...
function fitContextBudget(texts, maxChars) {
	...
	for (let index = texts.length - 1; index >= 0; index -= 1) {
		...
		const room = Math.max(0, remaining - label.length);
...
async function runOptimize(ctx, cfg, template, text, contextText, signal) {
	...
	return { text: guarded.text, warnings: guarded.warnings, usage: usageSummary(usage) };
}
