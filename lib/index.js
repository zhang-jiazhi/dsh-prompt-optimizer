...
export function buildConfigSchema(z) {
	return z.object({
		...
		reasoningEffort: z.string().default('inherit').description('推理强度：inherit=不指定，由模型/适配器默认决定（推荐；优化是轻量任务，跟随主模型的 max 档会明显变慢）；off/low/medium/high/max 为显式覆盖；模型不支持该档位时本次优化自动回退到模型默认档位重试'),
		maxTokens: z.natural().min(MIN_MAX_TOKENS).max(MAX_MAX_TOKENS).default(8192).description('单次输出 token 上限（推理型模型会把思考 token 计入同一预算，留足余量；预算被思考吃光时本插件会按更宽预算自动重试一次）'),
		timeoutMs: z.natural().min(MIN_TIMEOUT_MS).default(120000).description('单次优化超时（毫秒）'),
		maxInputChars: z.natural().min(MIN_MAX_INPUT_CHARS).max(MAX_MAX_INPUT_CHARS).default(8000).description('待优化文本长度上限（字符，超长拒绝）'),
		contextMaxMessages: z.natural().max(MAX_CONTEXT_MAX_MESSAGES).default(12).description('上下文类模板携带的最近对话条数'),
		contextMaxChars: z.natural().max(MAX_CONTEXT_MAX_CHARS).default(4000).description('上下文文本总长上限（字符；0=关闭上下文携带；预算不足时优先保留最新对话）'),
...
/**
 * 设置命名空间注册的完成信号（测试钩子）：只在 settings 服务出现后才会被
 * 赋值为动态导入的 promise；服务缺失时保持已 resolve，不会挂住调用方。
 */
...
function apply(ctx) {
	...
	// schemastery 是唯一外部依赖：动态导入，缺失时只降级设置页（走内置默认
	// 值），不再让整个插件树加载失败；优化主功能不受影响。
	let scope = null;
	ctx.inject(['settings'], (sctx) => {
		const settings = sctx.settings;
		if (!settings || typeof settings.register !== 'function') return;
		settingsReady = import('@deepseek-ai/schemastery')
			.then((module) => {
				const z = module.default ?? module;
				scope = settings.register('dsh-prompt-optimizer', buildConfigSchema(z), {});
			})
			.catch((error) => {
				ctx.logger?.warn?.('dsh-prompt-optimizer: 无法加载设置依赖 @deepseek-ai/schemastery，使用内置默认配置（优化功能不受影响）', error);
			});
	});
	const config = () => {
		try {
