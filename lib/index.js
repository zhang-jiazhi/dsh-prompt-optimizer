...
export const Config = z.object({
	...
	reasoningEffort: z.string().default('inherit').description('推理强度：inherit=跟随模型默认（最稳），off/low/medium/high/max 可加速；配置的模型不支持时自动忽略'),
	maxTokens: z.natural().default(2048).description('单次输出 token 上限'),
 ...
 * 读不到时返回空串，模板照常运行（上下文段落替换为占位说明）。
 */
async function gatherContext(ctx, sessionId, cfg, signal) {
	if (!sessionId || cfg.contextMaxMessages <= 0 || cfg.contextMaxChars <= 0) return '';
	const sessionQuery = ctx.get('sessionQuery');
	const readSurface = typeof sessionQuery?.readSurface === 'function';
	if (!readSurface && typeof sessionQuery?.readSession !== 'function') return '';
	try {
		...
		const texts = collected
			...
			.map(({ role, text }) => {
				...
				return `[${role === 'user' ? '用户' : '助手'}] ${clipped}`;
			});
		if (texts.length === 0) return '（无，本消息是本次对话的第一条消息）';
...
async function runOptimize(ctx, cfg, template, text, contextText, signal) {
	...
	// 推理强度设置：inherit=跟随模型默认；显式档位覆盖（模型不支持时由适配层报错/忽略）。
	...
	if (kind === 'max-tokens') throw new Error(`输出达到 token 上限（${cfg.maxTokens}），可在设置页调大「单次输出 token 上限」`);
	...
	ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			const cfg = {
				...
				maxTokens: positiveIntOr(raw.maxTokens, 2048),
			...
			try {
				...
				const contextText = template.category === 'context'
				...
				const optimized = await withAbort(
					runOptimize(ctx, cfg, template, text, contextText, deadline.signal),
					deadline.signal,
				);
				writeJson(res, 200, {
					ok: true,
